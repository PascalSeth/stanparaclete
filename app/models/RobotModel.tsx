...
const OverlayItem: React.FC<OverlayItemProps> = ({
  ...
  return (
    <Html
      ...
      distanceFactor={1.5}
      center
      className={`w-60 rounded-md overflow-hidden ${
    ...
    >
      <div className='bg-white bg-opacity-50 backdrop-blur-lg text-xs p-3 w-full'>
        ...
        <p className='text-[16px] p-[1]'>{description}</p>
      </div>
      <Link href={linkTo}>
        <button
          className={`${bgColor} hover:cursor-pointer hover:bg-opacity-50 transition-colors duration-500 px-4`}
        >
          Explore
        </button>
      </Link>
...
export function RobotModel( {html,...props}:RobotModelProps) {
  ...
  return (
    <group {...props} dispose={null}>
      ...
      <OverlayItem
  title="Creations"
  description="Explore our creations"
  bgColor="bg-blue-500"
  linkTo="/Creation"
...
{html &&(
      <OverlayItem
  title="Resources"
  description="Check out our various projects and resources."
  bgColor="bg-blue-500"
  linkTo="/about"
  rotation-x={degToRad(180)}
  rotation-z={degToRad(180)}
